...
def print_metrics(
    ...
    for Xs, Ys, Ys_aux, partition in datasets:
        ...
        if loss_type == 'dfl':
            ...
            Ys_aux_copy = Ys_aux.detach().cpu()
            Ys_copy = Ys.detach().cpu()
            Zs_pred = problem.get_decision(pred_copy, aux_data=Ys_aux_copy, data_partition = partition, data_num = -1,isTrain=isTrain)
            objectives = problem.get_objective(Ys_copy, Zs_pred, aux_data=Ys_aux_copy)
            
        elif loss_type == 'mse':
            pred = model(Xs).squeeze()
            pred_copy = pred.detach().cpu()
            Ys_copy = Ys.detach().cpu()
            objectives = torch.sqrt(torch.square(pred_copy-Ys_copy))
        ...
        #     if wandb_on:
        #         wandb.log({
        #             f"Obj_{partition}": objective,
        #             f"Loss_{partition}": loss,
        #             # f"MAE_{partition}": std,
        #         })
